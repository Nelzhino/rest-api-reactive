package com.reactor.app.handler;
...
public class ProductoHandler {
	...
	public Mono<ServerResponse> list(ServerRequest reuqest) {
		...
		return producto.flatMap(p -> {
			...
			if (errors.hasErrors()) {
				return Flux.fromIterable(errors.getFieldErrors())
						...
						.collectList().flatMap(list -> ServerResponse.badRequest().body(BodyInserters.fromValue(list)));
			} else {
				if (p.getCreateAt() == null) {
					p.setCreateAt(new Date());
				}
				return service.save(p)
						.flatMap(pdb -> ServerResponse.created(URI.create("/api/v2/producto".concat(p.getId())))
	...
	public Mono<ServerResponse> createWithPhotos(ServerRequest request) {
